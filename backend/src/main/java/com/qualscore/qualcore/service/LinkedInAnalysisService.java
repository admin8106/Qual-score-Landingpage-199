...
public interface LinkedInAnalysisService {
    ...
    /**
     * Analyze a LinkedIn profile from the HTTP API request DTO.
     * Adapts the request to a {@link LinkedInProfileInput} and runs analysis (no persistence).
     *
     * @param request the incoming HTTP API request
     * @return the HTTP API response DTO
     */
    AnalyzeLinkedInResponse analyzeProfile(AnalyzeLinkedInRequest request);
}
