...
public class AdminAuthServiceImpl implements AdminAuthService {
    ...
    public AdminLoginResponse login(AdminLoginRequest request) {
        AdminUser admin = adminUserRepository.findByEmailIgnoreCase(request.email())
                .orElseThrow(() -> {
                    ...
                    return new BusinessException("INVALID_CREDENTIALS", INVALID_CREDENTIALS_MSG, org.springframework.http.HttpStatus.UNAUTHORIZED);
                });

        if (!admin.isActive()) {
            log.warn("[AdminAuth] Login attempt for inactive admin email={}", request.email());
            throw new BusinessException("INVALID_CREDENTIALS", INVALID_CREDENTIALS_MSG, org.springframework.http.HttpStatus.UNAUTHORIZED);
        }
    ...
    public AdminProfileResponse getProfile(String email) {
        AdminUser admin = adminUserRepository.findByEmailIgnoreCase(email)
                .orElseThrow(() -> new BusinessException("ADMIN_NOT_FOUND", "Admin not found", org.springframework.http.HttpStatus.NOT_FOUND));
