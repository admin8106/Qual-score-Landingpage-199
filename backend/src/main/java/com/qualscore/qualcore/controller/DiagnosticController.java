...
public class DiagnosticController {
    ...
    @GetMapping("/analyze/{candidateReference}/status")
    @Operation(summary = "Get diagnostic analysis status",
               description = "Returns the current analysis status for a candidate: whether analysis is complete, whether a report has been generated, the band label, final score, and when the analysis was last run.")
    public ResponseEntity<ApiResponse<DiagnosticAnalysisStatusResponse>> getAnalysisStatus(
            @PathVariable String candidateReference) {
        return ResponseEntity.ok(ApiResponse.success(orchestrationService.getAnalysisStatus(candidateReference)));
    }
