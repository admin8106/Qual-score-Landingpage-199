...
async function trackEvent(request: LogAnalyticsEventRequest): Promise<ApiResult<LogAnalyticsEventResponse>> {
  return httpClient.post<LogAnalyticsEventResponse>(`${BASE}/event`, request);
}
...
export const analyticsApi = {
  ...
  fire(
    ...
    anonymousId: string,
    occurredAt: string,
    properties?: LogAnalyticsEventRequest['properties'],
  ): void {
    trackEvent({ eventName, anonymousId, occurredAt, properties }).catch(() => {});
  },
};
