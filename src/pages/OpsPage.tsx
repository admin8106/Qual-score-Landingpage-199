...
function applyQuickFilter(leads: AdminLeadRecord[], filter: QuickFilter): AdminLeadRecord[] {
  switch (filter) {
    ...
    case 'not_booked':
      return leads.filter((l) => !l.consultationBooked && !(!!l.consultationStatus && l.consultationStatus !== 'NONE' && l.consultationStatus !== 'none'));
    default:
      return leads;
  }
}
...
function NoteModal({
  ...
  const save = async () => {
    ...
    const { data, error } = await supabase
      .from('ops_actions')
      .insert({
        candidate_code: lead.candidateReference,
        action_type: 'note',
        note: trimmed,
  ...
  return (
    <div className="bg-white/[0.02] border border-white/[0.07] rounded-2xl overflow-hidden hover:border-white/[0.12] transition-colors">
          ...
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <p className="text-xs text-slate-500 truncate mt-0.5">
                ...
                {lead.totalExperienceYears ? ` · ${lead.totalExperienceYears}` : lead.yearsExperience != null ? ` · ${lead.yearsExperience}y` : ''}
              </p>
            </div>
      ...
      <div className="flex items-center flex-wrap gap-2 px-4 pb-3">
        ...
        {(lead.consultationBooked || (!!lead.consultationStatus && lead.consultationStatus !== 'NONE' && lead.consultationStatus !== 'none')) && <StatusPill label="Booked" color="amber" />}
      ...
      <div className="flex items-center gap-2 px-4 pb-4 border-t border-white/[0.05] pt-3">
        ...
        <button
          onClick={onBooked}
          className={`flex-1 inline-flex items-center justify-center gap-1.5 text-xs font-medium rounded-xl px-3 py-2 transition-colors ${
            actions.booked || lead.consultationBooked || (!!lead.consultationStatus && lead.consultationStatus !== 'NONE' && lead.consultationStatus !== 'none')
        ...
        >
          ...
          {actions.booked || lead.consultationBooked || (!!lead.consultationStatus && lead.consultationStatus !== 'NONE' && lead.consultationStatus !== 'none') ? 'Booked' : 'Mark Booked'}
        </button>
        <button
          onClick={onNote}
...
export default function OpsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    setError('');
    const result = await adminApi.getLeads({ limit: 500 }, token);
    if (!result.ok) {
  ...
  const markAction = useCallback(async (
    ...
    const key = lead.candidateReference;
  ...
  return (
    <div className="min-h-screen bg-[#080C18]">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {filtered.map((lead) => (
              <LeadCard
                key={lead.candidateReference}
                lead={lead}
                actions={opsActions[lead.candidateReference] ?? { contacted: false, booked: false, notes: [], lastActionAt: null }}
      ...
      {noteModalLead && (
        <NoteModal
          lead={noteModalLead}
          existingNotes={opsActions[noteModalLead.candidateReference]?.notes ?? []}
