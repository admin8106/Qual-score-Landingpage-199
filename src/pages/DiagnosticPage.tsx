...
export default function DiagnosticPage() {
  ...
  const fetchQuestions = useCallback(async () => {
    ...
    const mappedData = result.ok && result.data.length > 0
      ? mapBackendQuestions(result.data)
      : null;
    ...
    const validCodes = new Set(mappedData.questions.map((q) => q.questionCode));

    setMapped(mappedData);

    setLocalAnswers((prev) => {
      const valid = prev.filter((a) => validCodes.has(a.questionCode));
      const resumeIndex = valid.length > 0 ? Math.min(valid.length, mappedData.total - 1) : 0;
  ...
  const [localAnswers, setLocalAnswers] = useState<LocalAnswer[]>(() => {
    ...
    try {
      ...
      if (raw) return JSON.parse(raw) as LocalAnswer[];
    } catch { /* ignore */ }
    return [];
  });
