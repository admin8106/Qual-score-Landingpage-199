...
export function applyFilters(
  ...
  return leads.filter((l) => {
    ...
    if (filters.consultationStatus === 'booked' && !l.consultationBooked) return false;
    if (filters.consultationStatus === 'not_booked' && l.consultationBooked) return false;
...
export function computeStats(leads: AdminLeadRecord[]): AdminStats {
  ...
  for (const l of leads) {
    ...
    if (l.consultationBooked) consultationBookings++;
    const tags = l.tags ?? [];
