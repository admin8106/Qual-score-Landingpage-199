...
import { schedulingProviderApi, type SchedulingProviderConfig, type GenericTestLog } from '../../api/services/genericProviders';
import GenericProviderCard, { type ProviderMeta } from './GenericProviderCard';
import SchedulingProviderDrawer from './SchedulingProviderDrawer';

const SCHEDULING_META: ProviderMeta = {
  calendly:        { label: 'Calendly',          color: 'text-rose-400',   bg: 'bg-rose-500/10',   abbr: 'CL' },
  google_calendar: { label: 'Google Calendar',   color: 'text-blue-400',   bg: 'bg-blue-500/10',   abbr: 'GC' },
  stub:            { label: 'Stub (Internal)',   color: 'text-slate-400',  bg: 'bg-slate-500/10',  abbr: 'SB' },
  custom:          { label: 'Custom',            color: 'text-slate-400',  bg: 'bg-slate-500/10',  abbr: 'CU' },
};

interface Props { adminEmail: string; adminRole?: string; }

export default function SchedulingManagementPanel({ adminEmail, adminRole = 'VIEWER' }: Props) {
  const [providers, setProviders] = useState<SchedulingProviderConfig[]>([]);
  const [logMap, setLogMap]       = useState<Record<string, GenericTestLog>>({});
  const [loading, setLoading]     = useState(true);
  const [error, setError]         = useState('');
  const [selected, setSelected]   = useState<SchedulingProviderConfig | null>(null);
  const [showNew, setShowNew]     = useState(false);

  const load = useCallback(async () => {
    setLoading(true); setError('');
    try {
      const provs = await schedulingProviderApi.list();
      setProviders(provs);
      const map: Record<string, GenericTestLog> = {};
      await Promise.all(provs.map(async (p) => {
        const logs = await schedulingProviderApi.getTestLogs(p.id);
  ...
  function handleSaved(p: SchedulingProviderConfig) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-base font-semibold text-white">Scheduling Provider Management</h2>
          <p className="text-xs text-slate-500 mt-0.5">Configure booking and calendar integrations for consultation scheduling.</p>
        </div>
      ...
      {loading && <div className="flex items-center justify-center py-14"><div className="w-6 h-6 border-2 border-rose-500/30 border-t-rose-500 rounded-full animate-spin" /></div>}
      ...
      {!loading && !error && (
        <>
          <div className="flex justify-end mb-4">
            <button onClick={() => setShowNew(true)} className="inline-flex items-center gap-1.5 text-xs text-white bg-rose-700 hover:bg-rose-600 rounded-lg px-3 py-1.5 transition-colors font-medium">
              <Plus className="w-3.5 h-3.5" /> Add Provider
            </button>
          </div>
          {providers.length === 0 ? (
            <div className="text-center py-14 text-slate-600"><Calendar className="w-9 h-9 mx-auto mb-3 opacity-25" /><p className="text-sm">No scheduling providers configured.</p><p className="text-xs mt-1">Add Calendly or Google Calendar to enable real-time booking.</p><button onClick={() => setShowNew(true)} className="mt-4 inline-flex items-center gap-2 text-xs text-rose-400 hover:text-rose-300 transition-colors"><Plus className="w-3.5 h-3.5" /> Add provider</button></div>
          ) : (
            <div className="space-y-6">
              {liveGroup.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
                    {liveGroup.map((p) => <GenericProviderCard key={p.id} provider={p} meta={SCHEDULING_META} latestLog={logMap[p.id] ?? null} subtitle={p.booking_url ?? undefined} onClick={() => setSelected(p)} />)}
                  </div>
                </div>
              )}
              {sandboxGroup.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
                    {sandboxGroup.map((p) => <GenericProviderCard key={p.id} provider={p} meta={SCHEDULING_META} latestLog={logMap[p.id] ?? null} subtitle={p.booking_url ?? undefined} onClick={() => setSelected(p)} />)}
                  </div>
                </div>
              )}
            </div>
      ...
      {selected && <SchedulingProviderDrawer provider={selected} isNew={false} adminEmail={adminEmail} onClose={() => setSelected(null)} onSaved={handleSaved} onDeleted={handleDeleted} />}
      {showNew && <SchedulingProviderDrawer provider={null} isNew={true} adminEmail={adminEmail} onClose={() => setShowNew(false)} onSaved={handleSaved} onDeleted={() => {}} />}
    </div>
  );
}
