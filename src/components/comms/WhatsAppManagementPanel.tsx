...
import { Plus, RefreshCw, AlertTriangle, MessageCircle } from 'lucide-react';
import {
  whatsappProviderApi,
...
import WhatsAppProviderDrawer from './WhatsAppProviderDrawer';
...
export default function WhatsAppManagementPanel({ adminEmail, adminRole = 'VIEWER', appBaseUrl }: Props) {
  const [providers, setProviders]   = useState<WhatsAppProviderConfig[]>([]);
  const [logMap, setLogMap]         = useState<Record<string, CommTestLog>>({});
  const [loading, setLoading]       = useState(true);
  const [error, setError]           = useState('');
  const [selected, setSelected]     = useState<WhatsAppProviderConfig | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const provs = await whatsappProviderApi.list();
      setProviders(provs);
      const map: Record<string, CommTestLog> = {};
      await Promise.all(provs.map(async (p) => {
        const logs = await whatsappProviderApi.getTestLogs(p.id);
  ...
  function handleSaved(p: WhatsAppProviderConfig) {
    setProviders((prev) => {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-base font-semibold text-white">WhatsApp Provider Management</h2>
          <p className="text-xs text-slate-500 mt-0.5">
            Configure WhatsApp Business API providers. Meta Cloud API is the primary supported integration.
          </p>
        </div>
          ...
          <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl px-4 py-3">
            {primary ? (
              <>
                ...
                <p className="text-xs text-slate-600 mt-0.5">{primary.environment_mode} · Primary</p>
              </>
            ) : (
      ...
      {!loading && !error && (
        <>
          <div className="flex justify-end mb-4">
            <button onClick={() => setShowNew(true)} className="inline-flex items-center gap-1.5 text-xs text-white bg-green-700 hover:bg-green-600 rounded-lg px-3 py-1.5 transition-colors font-medium">
          ...
          {providers.length === 0 ? (
            <div className="text-center py-14 text-slate-600">
              <MessageCircle className="w-9 h-9 mx-auto mb-3 opacity-25" />
              <p className="text-sm">No WhatsApp providers configured.</p>
              <p className="text-xs mt-1">Add Meta Cloud API to start sending messages.</p>
              <button onClick={() => setShowNew(true)} className="mt-4 inline-flex items-center gap-2 text-xs text-green-400 hover:text-green-300 transition-colors">
                <Plus className="w-3.5 h-3.5" /> Add provider
              </button>
            </div>
          ) : (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
                    {liveGroup.map((p) => (
                      <CommProviderCard
                        ...
                        subtitle={p.sender_phone_display ?? undefined}
