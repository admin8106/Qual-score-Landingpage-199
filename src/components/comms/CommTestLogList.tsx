...
export default function CommTestLogList({ logs, loading }: Props) {
  if (loading) {
    return (
  ...
  if (logs.length === 0) {
    return (
      <div className="text-center py-12 text-slate-600">
        ...
        <p className="text-xs mt-1">Use "Test Config" to validate credentials.</p>
      </div>
    );
  }
      ...
      {logs.map((log) => (
        <div key={log.id} className="bg-white/[0.02] border border-white/[0.06] rounded-xl px-4 py-3 space-y-3">
          <div className="flex items-center gap-2">
            ...
            {log.test_recipient && (
              <span className="text-xs text-slate-600">→ {log.test_recipient}</span>
            )}
