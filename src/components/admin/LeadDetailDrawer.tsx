...
export default function LeadDetailDrawer({ lead, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    adminApi.getLead(lead.candidateReference).then((result) => {
      if (cancelled) return;
      if (result.ok) {
    ...
    adminApi.getComms(lead.candidateReference).then((result) => {
      if (cancelled) return;
  ...
  const handleResend = useCallback(async () => {
    ...
    setResendMsg('');
    const result = await adminApi.resend(lead.candidateReference);
    if (!mountedRef.current) return;
    setResending(false);
    if (result.ok) {
      setResendMsg('Resend triggered. Messages will arrive within a few seconds.');
      const commsResult = await adminApi.getComms(lead.candidateReference);
      if (!mountedRef.current) return;
  ...
  }, [lead.candidateReference, resending, resendCooldown]);
    ...
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-xl z-50 bg-[#0D1120] border-l border-white/[0.08] flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-2 border-b border-white/[0.04] shrink-0">
          ...
          <span className="font-mono text-xs text-slate-400">{src.candidateReference}</span>
          {src.leadPriority && (
